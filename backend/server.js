...
app.use(function (req, res, next) {
    ...
    res.setHeader('Access-Control-Allow-Headers', 'Origin, Accept, X-Requested-With, Content-type, Authorization, Token-Type, User-Info, x-access-token, c-access-token');
...
app.get('/openbill/:id', (req,res)=> {
        let id = req.body.id;
    con.query('SELECT * FROM open_bill WHERE id = ?',[id] , (error , results , fields)=>{
        console.log(results);
        if (error) throw error;
...
app.post('/openbill', (req,res)=>{
    ...
    if(  !codeProduct || !amount || !price ){
        ...
        con.query('INSERT INTO open_bill(  codeProduct , amount , price , comment ) VALUES(?,?,?,?)', [codeProduct,amount,price,comment] ,(error,result,fields)=>{
            if (error) throw error;
...
app.post('/address', (req,res)=>{
    ...
    if(  !name || !tambon || !amphur || !province || !zipCod || !phone ){
        ...
        con.query('INSERT INTO address(  name , tambon , amphur , province , zipCod , phone) VALUES(?,?,?,?,?,?)', [ name , tambon , amphur , province , zipCod , phone] ,(error,result,fields)=>{
            if (error) throw error;
            return res.send({ error: false, data: result , message: "success!!" })
        })
    }
...
app.put('/transfer',(req , res )=>{
    ...
    if(!id || !bank || !createdate || !payAmount || !paySlip){
        ...
        con.query('UPDATE transfer SET bank = ? , createdate =? , payAmount = ? , paySlip=? WHERE id = ?' , [bank,createdate,payAmount,paySlip,id],(error,result,fields)=>{
            if (error) throw error;
...
app.put('/address', (req,res)=>{
    ...
    if(  !name || !tambon || !amphur || !province || !zipCode || !phone || !address){
        return res.status(400).send({error: true , mesage : "dont success"});
    }
    else{
        con.query('UPDATE address SET address= ? ,name= ?  ,tambon= ? , amphur= ? , province= ? , zipCode= ? , phone= ? WHERE id = ?', [ address ,name , tambon , amphur , province , zipCode , phone , id] ,(error,result,fields)=>{
            if (error) throw error;
            return res.send({ error: false, data: result , message: "success!!" })
        })
    }
...
app.delete('/openbill',(req,res)=>{
    ...
    if(!id){
        ...
        con.query('DELETE FROM open_bill WHERE id=?',[id],(error,result,fields)=>{
            if(error) throw error;
